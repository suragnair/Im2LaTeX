...
model_name = 'msl20_50x280_cnn3_gru_attn_lr_e-3_e200_mem1000'
...
def train_batch(dat_x, dat_y):
    ...
    _, cnn_fc_t, cnn_attn_t, pred_t, loss_t, acc_t, summary = sess.run([train_op, cnn_fc, cnn_attn, pred, loss, accuracy, summary_op], feed_dict)
    return cnn_fc_t, cnn_attn_t, pred_t, loss_t, acc_t, summary

saver = tf.train.Saver()
#load_model_path = model_path + 'msl20_50x280_cnn3_gru_attn_lr_e-3_e200_mem1000_29_0.468536359.ckpt'
...
for epoch in range(training_epochs):
    ...
    while i + batch_size < len(train_list):
        i, dat_x, dat_y = gen_next_batch(0)
        cnn_fc_t, cnn_attn_t, pred_t, loss_t, acc_t, summary = train_batch(dat_x, dat_y)
	...
	print('=================================')
	for i in range(10):
	    print('gold : ' + ' '.join([invv[x] for x in dat_y[i]]))
	    print('pred : ' + ' '.join([invv[x] for x in pred_t.T[i]]))
	    print('-----')
	print('FC layer')
	print('--------')
	for i in range(5):
	...
	print('ATTN layer')
	for i in range(5):
	    print(cnn_attn_t[i][400][100:120])
	print('====================')
        sys.stdout.flush()
