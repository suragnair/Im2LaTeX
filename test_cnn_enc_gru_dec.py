...
def test_batch(dat_x, dat_y):
    ...
    pred_t, loss_t, acc_t, summary = sess.run([pred, loss, accuracy, summary_op], feed_dict)
    return pred_t, loss_t, acc_t, summary

saver = tf.train.Saver()
load_model_path = model_path  +  'cnn3_gru_lr_1e-3_e200_mem1000_9_0.528349138.ckpt'
...
for epoch in range(training_epochs):
    ...
    while i + batch_size < len(train_list):
        ...
        pred_t, loss_t, acc_t, summary = test_batch(dat_x, dat_y)
        ...
        for i in range(pred_t.shape[0]):
            ...
            print('predict : ' + ' '.join([invv[x] for x in pred_t[i]]))
