...
model_name = 'msl60_cnn3_gru_attn_lr_1e-2_e200_mem1000'
...
train_list_file = '/home/ee/btech/ee1130504/ELL881/p/data/snair_test/im2latex_test_snair.lst'
train_png_folder = '/home/ee/btech/ee1130504/ELL881/p/code/snair_test/snair_test_images/'
formulas_file = '/home/ee/btech/ee1130504/ELL881/p/data/snair_test/im2latex_formulas.norm.lst'
...
dec_outputs, dec_memory, dec_attn_coeffs = seq2seq_snair.embedding_attention_decoder(dec_inp, cnn_fc, cnn_attn, cell, vocab_size, embedding_dim, feed_previous=pred)
...
def test_batch(dat_x, dat_y):
    X = np.array(dat_x)	# batch_size * n_input
    Y = np.array(dat_y).T # max_seq_length * batch_size

    feed_dict = {x_input: X}
    feed_dict.update({keep_prob:1.})
    feed_dict.update({labels[t]: Y[t] for t in range(max_seq_length)})

    dec_attn_coeffs_t, cnn_fc_t, cnn_attn_t, pred_t, loss_t, acc_t, summary = sess.run([dec_attn_coeffs, cnn_fc, cnn_attn, pred, loss, accuracy, summary_op], feed_dict)
    return dec_attn_coeffs_t, cnn_fc_t, cnn_attn_t, pred_t, loss_t, acc_t, summary

saver = tf.train.Saver()
load_model_path = model_path  +  'msl100_50x280_cnn5_gru_attn_lr_1.00E-04_e300_mem1000_16_0.039367380.ckpt'
...
for epoch in range(training_epochs):
    ...
    while i + batch_size <= len(train_list):
        ...
        dec_attn_coeffs_t, cnn_fc_t, cnn_attn_t, pred_t, loss_t, acc_t, summary = test_batch(dat_x, dat_y)
        summary_writer.add_summary(summary, t)
        avg_loss += loss_t
        tot_cor += acc_t * len(dat_x)
        total_batch += 1
        pred_t = np.array(pred_t).T
        for j in range(pred_t.shape[0]):
            print('gold    : ' + ' '.join([invv[x] for x in dat_y[j] if (invv[x]!='</S>' and invv[x]!='<S>')]))
            print('predict : ' + ' '.join([invv[x] for x in pred_t[j] if (invv[x]!='</S>' and invv[x]!='<S>')]))
        print("Batch " + str(total_batch) + " loss = " + "{:.5f}".format(loss_t))
        print("Batch " + str(total_batch) + " acc  = " + "{:.5f}".format(acc_t))
        sys.stdout.flush()
	f = open('/home/ee/btech/ee1130504/dec_attn_coeffs.pkl','w')
	cp.dump((' '.join([invv[x] for x in pred_t[j] if (invv[x]!='</S>' and invv[x]!='<S>')]), np.array([[y for y in x[0][0]] for x in dec_attn_coeffs_t])), f)
	f.close()
	#pdb.set_trace()
        exit(0)
